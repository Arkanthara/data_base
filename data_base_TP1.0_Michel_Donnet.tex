...
$$\pi_{titre, \:année, \:nom \:réalisateur, \:durée}(\sigma_{(pays = France)}(Film))$$

\subsection{Les années de sortie des films réalisés par "Christopher Nolan"}
Voici la formule que je propose:
$$\sigma_{(nom \:réalisateur = Christopher \:Nolan)}(\pi_{année}(Film))$$
...
$$\pi_{titre}(Casting \bowtie_{(année - date \: naissance \: \geq 60)} Acteur)$$
\end{document}
